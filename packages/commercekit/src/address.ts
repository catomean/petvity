/**
 * Countries and postal addresses.
 *
 * Only the ISO 3166-1 alpha-2 codes are listed. Display names come from
 * `Intl.DisplayNames`, so every locale the runtime supports gets correctly
 * translated country names with no translation files to maintain and no chance
 * of the German list drifting from the English one. The codes themselves are an
 * international standard, which is why a literal list is config rather than a
 * magic array.
 */
