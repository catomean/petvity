...
function rowScore(
  row: Pick<HealthMetricRow, "mood" | "energy" | "anxiety" | "socialization">,
): number | null {
  const values: number[] = [];
...
function groupAvg(
  rows: Pick<HealthMetricRow, "mood" | "energy" | "anxiety" | "socialization">[],
): number | null {
 ...
 */
export function computeDigitalTwin(recentMetrics: HealthMetricRow[], now = new Date()): TwinState {
  ...
  const scored: Scored[] = [
    {
      id: "mood",
      raw: latest.mood,
      score: latest.mood != null ? ((latest.mood - 1) / 4) * 100 : -1,
    },
    {
      id: "energy",
      raw: latest.energy,
      score: latest.energy != null ? ((latest.energy - 1) / 4) * 100 : -1,
    },
    {
      id: "anxiety",
      raw: latest.anxiety,
      score: latest.anxiety != null ? ((5 - latest.anxiety) / 4) * 100 : -1,
    },
    {
      id: "socialization",
      raw: latest.socialization,
      score: latest.socialization != null ? ((latest.socialization - 1) / 4) * 100 : -1,
    },
  ];
  ...
  return {
    ...
    summaryKey: summaryKey(latest.mood, latest.energy, latest.anxiety, latest.socialization),
    metrics,
    daysAgo,
    trend,
    trendDelta,
