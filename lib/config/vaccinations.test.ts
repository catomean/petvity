...
describe("computeVaccinationDisplayStatus", () => {
  it("propagates not_applicable regardless of dates", () => {
    expect(computeVaccinationDisplayStatus("not_applicable", YESTERDAY, TODAY, DUE_SOON_DAYS)).toBe(
      "not_applicable",
    );
    expect(computeVaccinationDisplayStatus("not_applicable", null, TODAY, DUE_SOON_DAYS)).toBe(
      "not_applicable",
    );
    expect(
  ...
  it("returns up_to_date when nextDueDate is null (no booster needed)", () => {
    expect(computeVaccinationDisplayStatus("up_to_date", null, TODAY, DUE_SOON_DAYS)).toBe(
      "up_to_date",
    );
    // Ignores stale stored status too
    expect(computeVaccinationDisplayStatus("overdue", null, TODAY, DUE_SOON_DAYS)).toBe(
      "up_to_date",
    );
  });

  it("returns overdue when nextDueDate is in the past", () => {
    expect(computeVaccinationDisplayStatus("up_to_date", YESTERDAY, TODAY, DUE_SOON_DAYS)).toBe(
      "overdue",
    );
  });
  ...
  it("overrides stale up_to_date stored status when date has passed", () => {
    ...
    expect(computeVaccinationDisplayStatus("up_to_date", "2025-01-01", TODAY, DUE_SOON_DAYS)).toBe(
      "overdue",
    );
  });

  it("returns due_soon when nextDueDate is within the window", () => {
    expect(computeVaccinationDisplayStatus("up_to_date", IN_29_DAYS, TODAY, DUE_SOON_DAYS)).toBe(
      "due_soon",
    );
    // Exactly on the boundary
    expect(computeVaccinationDisplayStatus("up_to_date", TODAY, TODAY, DUE_SOON_DAYS)).toBe(
      "due_soon",
    );
  });

  it("returns up_to_date when nextDueDate is beyond the due-soon window", () => {
    expect(computeVaccinationDisplayStatus("up_to_date", IN_31_DAYS, TODAY, DUE_SOON_DAYS)).toBe(
      "up_to_date",
    );
  });
  ...
  it("honours a custom dueSoonDays window", () => {
    ...
    expect(computeVaccinationDisplayStatus("up_to_date", IN_29_DAYS, TODAY, 7)).toBe("up_to_date");
    // But IN_7_DAYS is due_soon
    const in7Days = "2026-05-01";
    expect(computeVaccinationDisplayStatus("up_to_date", in7Days, TODAY, 7)).toBe("due_soon");
  });
});
