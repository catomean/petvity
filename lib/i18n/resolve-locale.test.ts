...
describe("resolvePreferredLocale", () => {
  it("prefers a valid cookie over Accept-Language", () => {
    expect(
      resolvePreferredLocale({ cookieLocale: "ja", acceptLanguage: "de-DE,de;q=0.9" }),
    ).toBe("ja");
  });

  it("ignores an unknown cookie value and falls through", () => {
    expect(
      resolvePreferredLocale({ cookieLocale: "xx", acceptLanguage: "fr-FR" }),
    ).toBe("fr");
  });

  it("matches the primary subtag when only a regioned tag is sent (de-CH → de)", () => {
    expect(
      resolvePreferredLocale({ cookieLocale: null, acceptLanguage: "de-CH" }),
    ).toBe("de");
  });
  ...
  it("supports right-to-left locales (ar)", () => {
    expect(
      resolvePreferredLocale({ cookieLocale: null, acceptLanguage: "ar-SA" }),
    ).toBe("ar");
  });

  it("falls back to defaultLocale when nothing matches", () => {
    expect(
      resolvePreferredLocale({ cookieLocale: null, acceptLanguage: "xx-YY,zz" }),
    ).toBe("en");
  });
